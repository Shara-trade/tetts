...
# Добавляем папку admin в путь импорта
...
async def is_admin(user_id: int) -> bool:
    """Проверяет является ли пользователь админом (из БД, а не только env)"""
    # Создатель всегда админ
    ...
    try:
        ...
        async with db.lock:
            ...
            return row is not None and row[0] in ('admin', 'creator')
    except Exception:
        return False
