...
async def log_admin_action(admin_id: int, action: str, target_id: int = None, details: dict = None):
    ...
    db = await get_db()
    try:
        # Ограничиваем размер details до 1000 символов
        if details and isinstance(details, dict):
            details_str = json.dumps(details, ensure_ascii=False)[:1000]
        else:
            details_str = str(details)[:1000] if details else None
        
        await db.log_admin_action(admin_id, action, target_id, details_str)
    except Exception as e:
        logging.error(f"Failed to log admin action: {e}")


async def notify_admins(bot: Bot, message: str, priority: str = "normal", exclude_id: int = None):
    ...
    Args:
        ...
        exclude_id: ID админа, которому не отправлять
    """
    try:
        db = await get_db()
        admins = await db.fetchall(
            "SELECT user_id FROM admin_roles WHERE is_active = 1 AND role IN ('admin', 'creator')"
        )
        
        priority_emoji = {"low": "ℹ️", "normal": "🔔", "high": "🚨"}
        emoji = priority_emoji.get(priority, "🔔")
        full_message = f"{emoji} <b>УВЕДОМЛЕНИЕ</b>\n\n{message}"
        ...
        for admin in admins:
            ...
            try:
                ...
                # Админ заблокировал бота или удалил аккаунт
                pass
...
async def show_user_profile(message_or_callback, user_id: int):
    """Показывает профиль пользователя в админке с расширенной информацией"""
    try:
        db = await get_db()
        user = await db.get_user(user_id)
        ...
        try:
            plots_count = await db.fetchone(
                "SELECT COUNT(*) FROM plots WHERE user_id = ?", (user_id,)
            )
            plots_count = plots_count[0] if plots_count else 0
            
            total_harvests = await db.fetchone(
                "SELECT COUNT(*) FROM harvest_history WHERE user_id = ?", (user_id,)
            )
            total_harvests = total_harvests[0] if total_harvests else 0
            
            achievements_count = await db.fetchone(
                "SELECT COUNT(*) FROM player_achievements WHERE user_id = ?", (user_id,)
            )
            achievements_count = achievements_count[0] if achievements_count else 0
        except Exception:
            plots_count = total_harvests = achievements_count = "?"
        ...
        text += f"🌱 Грядок: {plots_count}\n"
        text += f"🌾 Сборов: {total_harvests}\n"
        text += f"🏆 Ачивок: {achievements_count}\n\n"
        
        text += f"🌾 Дата регистрации: {user.get('created_at', 'Неизвестно')[:10]}\n"
        text += f"⏱ Активность: {user.get('last_activity', 'Неизвестно')[:16]}\n"
        ...
        logging.error(f"Error showing user profile: {e}")
        error_text = "❌ <b>Ошибка при загрузке профиля!</b>"
        if isinstance(message_or_callback, CallbackQuery):
...
async def admin_last_players(callback: CallbackQuery):
    """Список последних зарегистрированных игроков"""
    db = await get_db()
    
    rows = await db.fetchall(
        """SELECT user_id, first_name, username, created_at, last_activity, balance
    ...
    # Формируем список последних игроков
    text = "📋 <b>ПОСЛЕДНИЕ 10 ИГРОКОВ</b>\n\n"
    
    for i, row in enumerate(rows, 1):
        user_id, first_name, username, created_at, last_activity, balance = row
        name = first_name or username or f"User_{user_id}"
        date_str = created_at[:10] if created_at else "?"
        text += f"{i}. {name} (ID: {user_id})\n"
        text += f"   📅 {date_str} | 💰 {balance:,}🪙\n\n"
    
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔄 Обновить", callback_data="admin_last_players")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="admin_players")]
    ])
...
async def admin_top_level(callback: CallbackQuery):
    ...
    rows = await db.fetchall(
        """SELECT user_id, first_name, username, level, xp, prestige_level 
           FROM users WHERE is_banned = 0 
    ...
    for i, row in enumerate(rows, 1):
        user_id, first_name, username, level, xp, prestige = row
        medal = {1: "🥇", 2: "🥈", 3: "🥉"}.get(i, f"{i}.")
        name = first_name or username or f"User_{user_id}"
        prestige_str = f" (П{prestige})" if prestige else ""
        text += f"{medal} {name}: Ур. {level} ({xp} XP){prestige_str}\n"
    
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔄 Обновить", callback_data="admin_top_level")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="admin_stats")]
    ])
...
async def admin_online_stats(callback: CallbackQuery):
    ...
    # Игроки, активные за последние 24 часа
    active_24h = await db.fetchone(
        "SELECT COUNT(*) FROM users WHERE last_activity >= datetime('now', '-1 day')"
    )
    
    # Игроки, активные за последний час
    active_1h = await db.fetchone(
        "SELECT COUNT(*) FROM users WHERE last_activity >= datetime('now', '-1 hour')"
    )
    ...
    # Всего игроков
    total = await db.fetchone("SELECT COUNT(*) FROM users")
    
    text = (
        "📊 <b>СТАТИСТИКА ОНЛАЙНА</b>\n\n"
        f"👥 Всего игроков: {total[0]}\n"
        f"🟢 Активны (1ч): {active_1h[0]}\n"
        f"🟡 Активны (24ч): {active_24h[0]}\n"
        f"🆕 Новые сегодня: {new_today[0]}\n\n"
        f"📈 Процент активных: {(active_24h[0] / total[0] * 100):.1f}%"
    )
...
async def admin_economy_menu(callback: CallbackQuery, state: FSMContext):
    ...
    total_coins = await db.fetchone("SELECT SUM(balance) FROM users")
    total_gems = await db.fetchone("SELECT SUM(gems) FROM users")
    avg_balance = await db.fetchone("SELECT AVG(balance) FROM users")
    
    text = (
        "💰 <b>УПРАВЛЕНИЕ ЭКОНОМИКОЙ</b>\n\n"
        f"📊 Общая статистика:\n"
        f"💵 Всего монет в обращении: {total_coins[0] or 0:,}🪙\n"
        f"💎 Всего самоцветов: {total_gems[0] or 0}💎\n"
        f"📈 Средний баланс: {int(avg_balance[0] or 0):,}🪙\n\n"
        "Выберите действие:"
    )

    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💸 Начислить всем", callback_data="eco_mass_give")],
        [InlineKeyboardButton(text="🔄 Списать у всех", callback_data="eco_mass_take")],
        [InlineKeyboardButton(text="📈 Инфляция", callback_data="eco_inflation")],
        [InlineKeyboardButton(text="📉 Дефляция", callback_data="eco_deflation")],
...
async def admin_help_menu(callback: CallbackQuery, state: FSMContext):
    """Меню помощи админу"""
    await state.clear()
    ...
    text = (
        ...
        "<b>Разделы панели:</b>\n"
        "👥 Игроки - Поиск, просмотр профилей, управление\n"
        "🌱 Растения - Управление семенами и ценами\n"
        "💰 Экономика - Массовые операции с валютой\n"
        "🎁 Промо-акции - Создание промокодов\n"
        "⭐️ Ежедневный бонус - Настройка бонусов\n"
        "🏆 Ачивки - Управление достижениями\n"
        "📢 Рассылка - Массовые сообщения\n\n"
        
        "<b>Поддержка:</b>\n"
        "При проблемах обращайтесь к @support"
    )
...
# ==================== РАЗДЕЛ «ЕЖЕДНЕВНЫЙ БОНУС» ====================

@router.callback_query(F.data == "admin_daily")
async def admin_daily_bonus_menu(callback: CallbackQuery, state: FSMContext):
    """Меню настройки ежедневного бонуса"""
    await state.clear()
    ...
    text = (
        ...
        "Настройка бонусов и серий.\n\n"
        "<b>Текущие настройки:</b>\n"
        "• Базовый бонус: 100-500🪙\n"
        "• Макс. серия: 30 дней\n"
        "• Множитель серии: x0.1 за день\n\n"
        "Выберите действие:"
    )
    
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⚙️ Настройки бонуса", callback_data="daily_settings")],
        [InlineKeyboardButton(text="📊 Статистика бонусов", callback_data="daily_stats")],
        [InlineKeyboardButton(text="🎁 Создать особый бонус", callback_data="daily_special")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="admin_back_main")],
    ])
...
async def admin_manage_admins_menu(callback: CallbackQuery, state: FSMContext):
    """Меню управления администраторами (только для Creator)"""
    await state.clear()
    ...
    db = await get_db()
    admins = await db.fetchall(
        "SELECT user_id, role, granted_by, granted_at FROM admin_roles WHERE is_active = 1"
    )
    
    text = (
        "👑 <b>УПРАВЛЕНИЕ АДМИНИСТРАТОРАМИ</b>\n\n"
        f"<b>Активных админов:</b> {len(admins)}\n\n"
    )

    if admins:
        text += "<b>Список:</b>\n"
        for admin in admins:
            admin_id, admin_role, granted_by, granted_at = admin
            role_emoji = get_role_emoji(admin_role)
            text += f"{role_emoji} ID: {admin_id} ({admin_role})\n"
            if granted_at:
                text += f"   Назначен: {granted_at[:10]}\n"
    
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="➕ Добавить админа", callback_data="admin_add_admin")],
        [InlineKeyboardButton(text="➖ Удалить админа", callback_data="admin_remove_admin")],
        [InlineKeyboardButton(text="📋 История назначений", callback_data="admin_history")],
        [InlineKeyboardButton(text="🔙 Назад", callback_data="admin_back_main")],
    ])
...
async def admin_logs_menu(callback: CallbackQuery, state: FSMContext):
    ...
    recent_logs = await db.fetchall(
        """SELECT al.created_at, al.admin_id, u.first_name, al.action, al.details 
           FROM admin_logs al
    ...
    text = (
        "📊 <b>ЛОГИ ДЕЙСТВИЙ АДМИНОВ</b>\n\n"
        "<b>Последние действия:</b>\n\n"
    )
    
    if recent_logs:
        for log in recent_logs:
            created_at, admin_id, admin_name, action, details = log
            text += f"🕐 {created_at[11:16]} | {admin_name or admin_id}\n"
            text += f"   {action}\n"
            if details:
                text += f"   Детали: {details[:50]}...\n"
            text += "\n"
    else:
        text += "Нет записей в логе.\n"
    
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📅 За сегодня", callback_data="logs_today")],
        [InlineKeyboardButton(text="📆 За неделю", callback_data="logs_week")],
...
async def admin_settings_menu(callback: CallbackQuery, state: FSMContext):
    """Меню настроек системы (только для Creator)"""
    await state.clear()
    ...
    text = (
        ...
        "• Комиссия рынка: 5%\n"
        "• Макс. грядок: 20\n"
        "• Кулдаун торговли: 24ч\n\n"
        "Выберите параметр для изменения:"
    )
    
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💰 Экономика", callback_data="settings_economy")],
        [InlineKeyboardButton(text="🎮 Игровые", callback_data="settings_game")],
