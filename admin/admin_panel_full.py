...
async def check_admin_access(user_id: int) -> str:
    """Проверяет доступ и возвращает роль (из БД или CREATOR_ID)"""
    import logging
    import os
    ...
    # Сначала проверяем CREATOR_ID из окружения (может быть несколько через запятую)
    creator_id_str = os.getenv("CREATOR_ID", "")
    if creator_id_str:
        creator_ids = [cid.strip() for cid in creator_id_str.split(",")]
        if str(user_id) in creator_ids:
            logger.info(f"[ADMIN] Пользователь user_id={user_id} распознан как CREATOR через env")
            return 'creator'
