...
Версия: 2.1 (исправлены дубликаты, добавлены импорты)
"""
...
from admin.states import AdminStates  # Используем единый класс состояний
...
async def notify_admins(bot: Bot, message: str, priority: str = "normal", exclude_id: int = None):
    ...
    try:
        ...
        admins = await db.fetchall(
            "SELECT user_id FROM admin_roles WHERE role IN ('admin', 'creator')"
        )
