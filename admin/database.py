...
# Дополнительные методы для админ-панели
    ...
    async def init_db(self):
        """Инициализирует базу данных - создаёт все таблицы и индексы"""
        import logging
        ...
        async with self.lock:
            ...
            await db.execute("""
                CREATE TABLE IF NOT EXISTS user_quests (
                    ...
                    completed_at TEXT,
                    FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE,
                    FOREIGN KEY (quest_id) REFERENCES quest_templates(quest_id) ON DELETE CASCADE,
            ...
            await db.execute("""
                CREATE TABLE IF NOT EXISTS achievements (
                    ...
                    reward_coins INTEGER DEFAULT 0,
                    reward_gems INTEGER DEFAULT 0,
                    reward_multiplier REAL DEFAULT 0.0,
                    icon TEXT DEFAULT '🏆',
                    sort_order INTEGER DEFAULT 0,
            ...
            await db.execute("""
                CREATE TABLE IF NOT EXISTS farmer_types (
                    ...
                    work_interval_seconds INTEGER DEFAULT 300,
                    is_active INTEGER DEFAULT 1,
                    sort_order INTEGER DEFAULT 0
                )
            """)
            ...
            await db.execute("""
                CREATE TABLE IF NOT EXISTS farmers (
                    ...
                    FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE,
                    FOREIGN KEY (farmer_type) REFERENCES farmer_types(type_code)
                )
            """)
            ...
            await db.execute("""
                CREATE TABLE IF NOT EXISTS farmer_config (
                    ...
                    preferred_crop TEXT,
                    harvest_mode TEXT DEFAULT 'sell',
                    use_fertilizer INTEGER DEFAULT 0,
                    auto_harvest INTEGER DEFAULT 1,
                    auto_plant INTEGER DEFAULT 1,
            ...
            await db.execute("""
                CREATE TABLE IF NOT EXISTS upgrades (
                    ...
                    required_prestige INTEGER DEFAULT 1,
                    is_active INTEGER DEFAULT 1,
                    sort_order INTEGER DEFAULT 0
                )
            """)
            ...
            await db.execute("""
                CREATE TABLE IF NOT EXISTS user_upgrades (
                    ...
                    updated_at TEXT DEFAULT CURRENT_TIMESTAMP,
                    FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE,
                    FOREIGN KEY (upgrade_code) REFERENCES upgrades(upgrade_code) ON DELETE CASCADE,
            ...
            await db.execute("""
                CREATE TABLE IF NOT EXISTS fertilizer_logs (
                    ...
                    time_reduced INTEGER,
                    new_time INTEGER,
                    income_bonus REAL DEFAULT 0.0,
            ...
            await db.execute("""
                CREATE TABLE IF NOT EXISTS daily_bonus_config (
                    ...
                    reward_gems INTEGER DEFAULT 0,
                    reward_item_code TEXT,
                    is_super_bonus INTEGER DEFAULT 0
                )
            """)
            ...
            await db.execute("CREATE INDEX IF NOT EXISTS idx_plots_user ON plots(user_id)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_plots_status ON plots(status)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_inventory_user ON inventory(user_id)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_inventory_item ON inventory(item_code)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_user_quests_user ON user_quests(user_id)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_achievements_user ON player_achievements(user_id)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_farmers_user ON farmers(user_id)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_promo_code ON promocodes(code)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_transfers_sender ON transfers(sender_id)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_transfers_receiver ON transfers(receiver_id)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_referrals_referrer ON referrals(referrer_id)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_event_leaderboard_event ON event_leaderboard(event_id)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_economy_logs_user ON economy_logs(user_id)")
            await db.execute("CREATE INDEX IF NOT EXISTS idx_economy_logs_timestamp ON economy_logs(timestamp)")
