...
class Database:
    ...
    async def get_weekly_quests(self, user_id: int) -> List[Dict]:
        ...
        quests = await self.fetchall(
            """SELECT q.quest_id, q.quest_type, q.target_item, q.target_count, 
                      ...
                      uq.progress, uq.completed, uq.claimed
               FROM quests q
        ...
        for q in quests:
            result.append({
                ...
                "target": q[3],
                "description": q[4],
                "name": q[4],  # description как name
                "reward_coins": q[5],
                "reward_gems": q[6],
                "reward_items": json.loads(q[7]) if q[7] else {},
                "progress": q[8] or 0,
                "completed": bool(q[9]),
                "claimed": bool(q[10]),
                "is_weekly": True
    ...
    async def get_active_quests(self, user_id: int) -> List[Dict]:
        """Получает все активные квесты пользователя (ежедневные + еженедельные)
        
        Args:
            user_id: ID пользователя
            
        Returns:
            Список квестов с прогрессом
        """
        daily = await self.get_daily_quests(user_id)
        weekly = await self.get_weekly_quests(user_id)
        
        # Объединяем и помечаем тип
        for q in daily:
            q["is_weekly"] = False
        for q in weekly:
            q["is_weekly"] = True
