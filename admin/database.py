...
# Дополнительные методы для админ-панели
    ...
    # ============ МЕТОДЫ ДЛЯ ADMIN PANEL (ACHIEVEMENTS) ============
    
    async def admin_get_achievement_stats(self) -> dict:
        """Получает статистику по ачивкам для админ-панели"""
        try:
            total = await self.fetchone("SELECT COUNT(*) FROM achievements")
            total_count = total[0] if total else 0
            active = await self.fetchone("SELECT COUNT(*) FROM achievements WHERE is_active = 1")
    ...
    async def admin_get_all_achievements(self, active_only: bool = False) -> list:
        """Получает список всех ачивок"""
        try:
            query = "SELECT code, name, description, category_id, is_active, icon FROM achievements"
            if active_only:
                query += " WHERE is_active = 1"
            rows = await self.fetchall(query + " ORDER BY sort_order, code")
            return [{'code': r[0], 'name': r[1], 'description': r[2], 'category_id': r[3], 'is_active': bool(r[4]), 'icon': r[5]} for r in rows]
        except Exception as e:
            import logging
    ...
    async def admin_create_achievement(self, data: dict) -> int:
        """Создает новую ачивку"""
        try:
            await self.execute("""
                INSERT INTO achievements (code, name, description, category_id, requirement_type, 
                requirement_count, is_secret, reward_coins, reward_gems, icon, sort_order, is_active)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                data['code'], data['name'], data.get('description', ''), data.get('category_id', 'general'),
                data.get('requirement_type', 'custom'), data.get('requirement_count', 1),
                1 if data.get('is_secret') else 0, data.get('reward_coins', 0),
                data.get('reward_gems', 0), data.get('icon', '🏆'),
    ...
    async def admin_update_achievement(self, code: str, data: dict) -> bool:
        """Обновляет ачивку"""
        try:
            await self.execute("""
                UPDATE achievements SET is_active = ? WHERE code = ?
            """, (1 if data.get('is_active') else 0, code), commit=True)
            return True
        except Exception as e:
            import logging
    ...
    async def admin_delete_achievement(self, code: str, full_delete: bool = False) -> bool:
        """Удаляет или деактивирует ачивку"""
        try:
            if full_delete:
                await self.execute("DELETE FROM achievements WHERE code = ?", (code,), commit=True)
            else:
                await self.execute("UPDATE achievements SET is_active = 0 WHERE code = ?", (code,), commit=True)
            return True
        except Exception as e:
            import logging
    ...
    async def admin_give_achievement(self, admin_id: int, user_id: int, achievement_code: str) -> bool:
        """Выдает ачивку пользователю"""
        try:
            # Проверяем существование ачивки
            ach = await self.fetchone("SELECT achievement_id FROM achievements WHERE code = ?", (achievement_code,))
            if not ach:
                return False
            ...
            import logging
            logging.error(f"Error giving achievement: {e}")
            return False
