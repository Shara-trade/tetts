...
class Database:
    def __init__(self, db_path: str = "farm_v3.db"):
...
# Дополнительные методы для админ-панели
    ...
    async def get_user_activity_history(self, user_id: int, limit: int = 20) -> List[Dict]:
        ...
        # Проверяем существование таблицы economy_logs
        try:
            ...
            for r in eco_rows:
                action_text = ""
                if r[1] == 'earn':
                    action_text = f"💰 Получено: {int(r[3] or 0):,} {r[2]}"
                elif r[1] == 'spend':
                    action_text = f"💸 Потрачено: {int(r[3] or 0):,} {r[2]}"
                elif r[1] == 'harvest':
                    action_text = f"🌾 Собран урожай: +{int(r[3] or 0):,} {r[2]}"
                elif r[1] == 'plant':
        ...
        # Проверяем существование таблицы achievement_logs
        try:
    ...
    async def get_daily_bonus_info(self, user_id: int) -> Dict[str, Any]:
        ...
        async with self.lock:
            ...
            cursor = await conn.execute(
                """SELECT last_claim_date, streak, total_claimed 
                   FROM user_daily 
                   WHERE user_id = ?""",
                (user_id,)
            )
            ...
            if row:
                last_claim_date, streak, total_claimed = row
            else:
                # Создаем запись если нет
                await conn.execute(
                    """INSERT INTO user_daily (user_id, last_claim_date, streak, total_claimed)
                       VALUES (?, date('now', '-1 day'), 0, 0)""",
                    (user_id,)
                )
                await conn.commit()
            ...
            if last_claim_date:
                # Парсим дату
                try:
                    if ' ' in last_claim_date:
                        last_dt = datetime.strptime(last_claim_date, "%Y-%m-%d %H:%M:%S")
                    else:
                    ...
                    if time_diff < 86400:  # 24 часа
                        can_claim = False
                        time_left = int(86400 - time_diff)
                except:
                    pass
            
            # Определяем день бонуса (1-7)
            current_day = ((streak or 0) % 7) + 1
            
            # Награды по дням
            rewards = {
                1: {"coins": 100, "gems": 0, "icon": "💰"},
                2: {"coins": 150, "gems": 0, "icon": "💰"},
                3: {"coins": 200, "gems": 0, "icon": "💰"},
                4: {"coins": 300, "gems": 0, "icon": "💰"},
                5: {"coins": 500, "gems": 0, "icon": "💰"},
                6: {"coins": 0, "gems": 1, "icon": "💎"},
                7: {"coins": 1000, "gems": 5, "icon": "🎁"},
            }
            
            today_reward = rewards.get(current_day, {"coins": 100, "gems": 0, "icon": "💰"})
            ...
            return {
                ...
                "today_coins": today_reward["coins"],
                "today_gems": today_reward["gems"],
                "today_icon": today_reward["icon"],
                "today_reward_text": f"{today_reward['coins']}🪙" if today_reward["coins"] > 0 else f"{today_reward['gems']}💎"
            }
    ...
    async def claim_daily_bonus(self, user_id: int) -> Dict[str, Any]:
        ...
        Returns:
            Словарь с результатом и наградой
        """
        async with self.lock:
            conn = await self.connect()
            
            # Проверяем можно ли забрать
            info = await self.get_daily_bonus_info(user_id)
            if not info["can_claim"]:
                return {"success": False, "error": "bonus_already_claimed", "time_left": info["time_left"]}
            ...
            reward = rewards.get(current_day, {"coins": 100, "gems": 0})
            
            # Проверяем серию (если прошло больше 48 часов - сбрасываем)
            new_streak = 1
            last_claim = info["last_claim_date"]
            ...
            if last_claim:
                try:
                    if ' ' in last_claim:
                        ...
                        last_dt = datetime.strptime(last_claim, "%Y-%m-%d")
                    
                    now = datetime.now()
                    if (now - last_dt).total_seconds() < 172800:  # 48 часов
            ...
            # Начисляем награду
            if reward["coins"] > 0:
                await self.update_balance(user_id, reward["coins"])
            ...
            await conn.execute(
                """INSERT INTO daily_bonus_history (user_id, bonus_day, reward_type, reward_amount, streak_at_claim)
                   VALUES (?, ?, 'coins', ?, ?)""",
                (user_id, current_day, reward["coins"], new_streak)
            )
    ...
    async def get_user(self, user_id: int) -> Optional[Dict]:
        ...
        """
        row = await self.fetchone(
            """SELECT user_id, first_name, username, balance, gems, prestige_level, 
                      prestige_multiplier, city_level, total_harvested, total_planted,
                      total_earned, total_spent, joined_date, last_activity, is_banned, settings
        ...
        settings = row[15] if row[15] else {}
        ...
        return {
            ...
            'balance': row[3],
            'gems': row[4],
            'prestige_level': row[5],
            'prestige_multiplier': row[6],
            'city_level': row[7],
            'total_harvested': row[8],
            'total_planted': row[9],
            'total_earned': row[10],
            'total_spent': row[11],
            'joined_date': row[12],
    ...
    async def create_user(self, user_id: int, username: str = "", first_name: str = "Игрок") -> bool:
        ...
        try:
            await self.execute(
                """INSERT OR IGNORE INTO users 
                   (user_id, username, first_name, balance, gems, prestige_level, prestige_multiplier)
                   VALUES (?, ?, ?, 100, 0, 1, 1.0)""",
                (user_id, username, first_name),
                commit=True
            )
            
            # Создаём начальные грядки (3 бесплатные)
            for i in range(1, 4):
                await self.execute(
                    "INSERT OR IGNORE INTO plots (user_id, plot_number, status) VALUES (?, ?, 'empty')",
                    (user_id, i),
                    commit=True
                )
    ...
    async def update_user_settings(self, user_id: int, settings: Dict) -> bool:
        ...
        try:
            ...
            logging.error(f"Error updating settings for {user_id}: {e}")
            return False
    
    # ==================== СИСТЕМА ГРЯДОК ====================
    
    async def get_plots(self, user_id: int) -> List[Dict]:
        """Получает список грядок пользователя с информацией о культурах
        
        Args:
            user_id: ID пользователя
            
        Returns:
            Список грядок с информацией
        """
        rows = await self.fetchall(
            """SELECT p.plot_number, p.status, p.crop_type, p.planted_time, 
                      p.growth_time_seconds, p.fertilized, p.fertilizer_type, p.fertilizer_bonus,
                      s.item_name, s.item_icon
               FROM plots p
               LEFT JOIN shop_config s ON p.crop_type = s.item_code
               WHERE p.user_id = ?
        ...
        for row in rows:
            plot = {
                ...
                'crop_type': row[2],
                'planted_time': row[3],
                'growth_time_seconds': row[4],
                'fertilized': row[5] if row[5] else False,
                'fertilizer_type': row[6],
                'fertilizer_bonus': row[7] if row[7] else 0.0,
                'crop_name': row[8] if row[8] else row[2],
                'crop_icon': row[9] if row[9] else '🌱'
            }
            
            # Рассчитываем оставшееся время
            if plot['status'] == 'growing' and plot['planted_time']:
                try:
                    planted = dt.fromisoformat(plot['planted_time'])
                    elapsed = (dt.now() - planted).total_seconds()
                    remaining = max(0, plot['growth_time_seconds'] - int(elapsed))
                    plot['remaining_time'] = remaining
                    
                    # Если время вышло, обновляем статус
                    if remaining <= 0:
                        plot['status'] = 'ready'
                except:
                    plot['remaining_time'] = plot['growth_time_seconds']
            else:
    ...
    async def plant_crop(self, user_id: int, plot_number: int, crop_type: str, growth_time: int) -> bool:
        """Сажает культуру на грядку
        
        Args:
            user_id: ID пользователя
            plot_number: Номер грядки
            crop_type: Код культуры
            growth_time: Время роста в секундах
        ...
        try:
            await self.execute(
                """UPDATE plots SET 
                   ...
                   fertilizer_bonus = 0.0
                   WHERE user_id = ? AND plot_number = ? AND status = 'empty'""",
                (crop_type, growth_time, user_id, plot_number),
                commit=True
            )
            return True
        except Exception as e:
    ...
    async def harvest_plots(self, user_id: int, multiplier: float = 1.0) -> Dict:
        """Собирает урожай с готовых грядок
        ...
        if not ready_plots:
            return {"success": False, "message": "Нет готовых грядок"}
        
        total_earned = 0
        crops_info = []
        
        for plot in ready_plots:
            crop_data = await self.get_shop_item(plot['crop_type'])
            if crop_data:
                base_price = crop_data.get('sell_price', 0)
                bonus = plot.get('fertilizer_bonus', 0)
                earned = int(base_price * multiplier * (1 + bonus))
                total_earned += earned
                ...
                crops_info.append({
                    ...
                    'name': crop_data.get('name', plot['crop_type']),
                    'earned': earned
                })
        
        # Начисляем монеты
        if total_earned > 0:
            new_balance = await self.update_balance(user_id, total_earned)
        else:
            user = await self.get_user(user_id)
        ...
        await self.execute(
            """UPDATE users SET 
               ...
               total_earned = total_earned + ?
               WHERE user_id = ?""",
            (len(ready_plots), total_earned, user_id),
            commit=True
        )
        ...
        return {
            "success": True,
            "total": total_earned,
            "harvested_count": len(ready_plots),
            "crops": crops_info,
            "new_balance": new_balance
        }
    
    async def update_balance(self, user_id: int, amount: int) -> Optional[int]:
        ...
        async with self.lock:
            ...
            try:
                ...
                await db.execute(
                    "UPDATE users SET balance = ?, last_activity = datetime('now') WHERE user_id = ?",
                    (new_balance, user_id)
                )
                await db.commit()
    ...
    async def get_plot_fertilizer_status(self, user_id: int, plot_number: int) -> Optional[Dict]:
        """Получает статус грядки для применения удобрения
    ...
    async def get_available_fertilizers(self, user_id: int) -> List[Dict]:
        """Получает доступные удобрения из инвентаря
        ...
        """
        rows = await self.fetchall(
            """SELECT i.item_code, i.quantity, s.item_name, s.item_icon, 
                      s.effect_type, s.effect_value, s.description
               FROM inventory i
               JOIN shop_config s ON i.item_code = s.item_code
               WHERE i.user_id = ? AND s.category = 'fertilizer' AND i.quantity > 0""",
            (user_id,)
        )
    ...
    async def calculate_fertilized_time(self, remaining_time: int, effect_type: str, effect_value: float) -> Dict:
        """Рассчитывает эффект удобрения
        
        Args:
            remaining_time: Оставшееся время в секундах
            effect_type: Тип эффекта (speed, instant, income)
            effect_value: Значение эффекта
        ...
        if effect_type == 'instant':
            ...
            # Ускорение на процент
            reduction = int(remaining_time * effect_value)
            result['new_time'] = max(0, remaining_time - reduction)
        elif effect_type == 'income':
            # Бонус к доходу
            result['income_bonus'] = effect_value
        
        return result
    
    async def apply_fertilizer(self, user_id: int, plot_number: int, fertilizer_code: str) -> Dict:
        """Применяет удобрение к грядке
        ...
        # Проверяем грядку
        plot = await self.get_plot_fertilizer_status(user_id, plot_number)
        if not plot:
            return {"success": False, "message": "Грядка не найдена"}
        ...
        # Получаем информацию об удобрении
        fert = await self.get_shop_item(fertilizer_code)
        if not fert:
            return {"success": False, "message": "Удобрение не найдено"}
        
        # Проверяем наличие в инвентаре
        inv_item = await self.get_inventory_item(user_id, fertilizer_code)
        if not inv_item or inv_item.get('quantity', 0) <= 0:
            return {"success": False, "message": "Нет удобрений в инвентаре"}
        
        # Рассчитываем эффект
        effect_type = fert.get('effect_type', 'speed')
        effect_value = fert.get('effect_value', 0.5)
        
        # Рассчитываем оставшееся время
        if plot['planted_time']:
            planted = dt.fromisoformat(plot['planted_time'])
            elapsed = (dt.now() - planted).total_seconds()
            remaining_time = max(0, plot['growth_time_seconds'] - int(elapsed))
        else:
            remaining_time = plot['growth_time_seconds']
        
        result = await self.calculate_fertilized_time(remaining_time, effect_type, effect_value)
        ...
        if result['is_instant']:
            ...
            await self.execute(
                """UPDATE plots SET 
                   ...
                   fertilizer_bonus = ?
                   WHERE user_id = ? AND plot_number = ?""",
                (fertilizer_code, result['income_bonus'], user_id, plot_number),
                commit=True
            )
        else:
            # Обновляем время и бонус
            new_planted = dt.now() - (dt.now() - dt.now())  # Сбрасываем время
            actual_growth = result['new_time']
            ...
            await self.execute(
                """UPDATE plots SET 
                   ...
                   fertilizer_bonus = ?
                   WHERE user_id = ? AND plot_number = ?""",
                (actual_growth, fertilizer_code, result['income_bonus'], user_id, plot_number),
                commit=True
            )
        ...
        return {
            "success": True,
            "message": "Удобрение применено!",
            "fertilizer_name": fert.get('name', fertilizer_code),
            "fertilizer_icon": fert.get('icon', '🧪'),
            "crop_type": plot['crop_type'],
            "is_instant": result['is_instant'],
            "original_time": result['original_time'],
            "new_time": result['new_time'],
            "income_bonus": result['income_bonus']
        }
    
    # ==================== ЛОГИРОВАНИЕ И АЧИВКИ ====================
    
    async def log_economy(self, user_id: int, operation_type: str, currency_type: str,
                          amount: int, balance_after: int, source: str = None,
                          source_id: str = None, details: str = None) -> bool:
        """Логирует экономическую операцию
        
        Args:
            user_id: ID пользователя
            operation_type: Тип операции (earn, spend, transfer)
            currency_type: Валюта (coins, gems)
            amount: Сумма
        ...
        try:
            await self.execute(
                """INSERT INTO economy_logs 
                   (user_id, operation_type, currency_type, amount, balance_after, source, source_id, details)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
                (user_id, operation_type, currency_type, amount, balance_after, source, source_id, details),
                commit=True
            )
            ...
            logging.error(f"Error logging economy: {e}")
            return False
    
    async def check_and_update_achievements(self, user_id: int, achievement_type: str, 
                                             count: int = 1) -> List[Dict]:
        """Проверяет и обновляет достижения пользователя
        
        Args:
            user_id: ID пользователя
            achievement_type: Тип достижения (harvest, plant, balance, prestige, etc.)
            count: Значение для проверки
        ...
        current_values = {
            'harvest': user.get('total_harvested', 0),
            'plant': user.get('total_planted', 0),
            'balance': user.get('balance', 0),
            'prestige': user.get('prestige_level', 1),
            'spend': user.get('total_spent', 0),
            'earn': user.get('total_earned', 0),
            'gems_total': user.get('gems', 0)
        }
        
        # Обновляем статистику если нужно
        if achievement_type == 'harvest':
            current_values['harvest'] += count
        elif achievement_type == 'plant':
            current_values['plant'] += count
        elif achievement_type == 'earn':
            current_values['earn'] += count
        elif achievement_type == 'spend':
            current_values['spend'] += count
        
        current_value = current_values.get(achievement_type, count)
        
        # Получаем активные ачивки этого типа
        rows = await self.fetchall(
            """SELECT a.achievement_id, a.code, a.name, a.description, a.icon,
                      a.requirement_count, a.reward_coins, a.reward_gems
               FROM achievements a
               LEFT JOIN player_achievements pa ON a.achievement_id = pa.achievement_id 
                   AND pa.user_id = ?
               WHERE a.requirement_type = ? AND a.is_active = 1
               AND (pa.completed IS NULL OR pa.completed = 0)""",
            (user_id, achievement_type)
        )
        
        completed = []
        for row in rows:
            ach_id, code, name, desc, icon, required, reward_coins, reward_gems = row
            ...
            if current_value >= required:
                ...
                await self.execute(
                    """INSERT INTO player_achievements 
                       ...
                       progress = ?, completed = 1, completed_at = datetime('now')""",
                    (user_id, ach_id, required, required),
                    commit=True
                )
                ...
                completed.append({
                    ...
                    'description': desc,
                    'icon': icon or '🏆',
                    'reward_coins': reward_coins or 0,
                    'reward_gems': reward_gems or 0
                })
    ...
    async def get_pending_notifications(self, limit: int = 100) -> List[Dict]:
        """Получает отложенные уведомления для отправки
        ...
        """
        rows = await self.fetchall(
            """SELECT notification_id, user_id, type, message, created_at
               FROM notifications 
               WHERE sent = 0 
    ...
    async def register_referral(self, referred_id: int, referrer_id: int) -> Dict:
        ...
        Returns:
            Результат регистрации
        """
        # Проверяем что не приглашаем сами себя
        if referred_id == referrer_id:
            return {"success": False, "message": "Нельзя пригласить себя"}
        ...
        if existing:
            return {"success": False, "message": "Уже зарегистрирован как реферал"}
        ...
        try:
            ...
            await self.update_balance(referred_id, 50)
            
            # Выдаём бонус пригласившему
            await self.update_balance(referrer_id, 100)
            
            return {
                "success": True,
                "referrer_username": referrer.get('username', 'друг'),
                "referred_bonus": 50,
                "referrer_bonus": 100
            }
        except Exception as e:
            logging.error(f"Error registering referral: {e}")
            return {"success": False, "message": "Ошибка регистрации"}
    
    async def _give_referral_reward(self, referrer_id: int, referred_id: int, reward_type: str) -> Dict:
        """Выдаёт награду за реферала (внутренний метод)"""
        rewards = {
            'prestige1': {'coins': 100, 'gems': 0},
            'prestige5': {'coins': 500, 'gems': 1},
            'prestige10': {'coins': 1000, 'gems': 5}
        }
        ...
        try:
            # Выдаём награду
            if reward['coins'] > 0:
                await self.update_balance(referrer_id, reward['coins'])
            
            if reward['gems'] > 0:
                await self.update_gems(referrer_id, reward['gems'])
            
            # Записываем что награда выдана
            await self.execute(
                """INSERT INTO referral_rewards 
                   (referrer_id, referred_id, reward_type, reward_coins, reward_gems)
                   VALUES (?, ?, ?, ?, ?)""",
                (referrer_id, referred_id, reward_type, reward['coins'], reward['gems']),
                commit=True
            )
            ...
            logging.error(f"Error giving referral reward: {e}")
            return {'type': reward_type, 'coins': 0, 'gems': 0}
    ...
    async def get_daily_bonus_streak(self, user_id: int) -> Dict:
        ...
        row = await self.fetchone(
            "SELECT current_streak, last_claim_date FROM user_daily WHERE user_id = ?",
            (user_id,)
        )
        ...
        streak, last_claim = row[0], row[1]
        can_claim = True
        
        if last_claim:
            try:
                last_dt = dt.fromisoformat(last_claim) if 'T' in last_claim else dt.strptime(last_claim, '%Y-%m-%d')
                today = dt.now().date()
    ...
    async def get_active_event(self) -> Optional[Dict]:
        ...
        return {
            ...
            'start_date': row[4],
            'end_date': row[5],
            'multiplier': row[6] if row[6] else 1.0,
            'is_active': row[7]
        }
    
    async def update_event_score(self, user_id: int, score: int) -> bool:
        """Обновляет счёт пользователя в событии
        
        Args:
            user_id: ID пользователя
            score: Очки для добавления
        ...
        try:
            await self.execute(
                """INSERT INTO event_leaderboard (event_id, user_id, score)
                   VALUES (?, ?, ?)
                   ON CONFLICT(event_id, user_id) DO UPDATE SET
                   score = score + ?, updated_at = datetime('now')""",
                (event['event_id'], user_id, score, score),
                commit=True
            )
            return True
        except Exception as e:
    ...
    async def log_admin_action(self, admin_id: int, action: str, target_user_id: int = None,
                                target_entity_id: str = None, details: str = None) -> bool:
        """Логирует действие администратора
        ...
        try:
            await self.execute(
                """INSERT INTO admin_logs 
                   (admin_id, action_type, target_user_id, target_entity_id, details)
                   VALUES (?, ?, ?, ?, ?)""",
                (admin_id, action, target_user_id, target_entity_id, details),
                commit=True
            )
            return True
        except Exception as e:
    ...
    async def get_admin_role(self, user_id: int) -> Optional[str]:
        ...
        """
        # Проверяем кэш
        if user_id in self._admin_roles_cache:
            return self._admin_roles_cache[user_id]
    ...
    async def get_all_active_farmers(self) -> List[Dict]:
        """Получает всех активных фермеров для фоновой задачи"""
        rows = await self.fetchall(
            """SELECT uf.user_id, uf.farmer_id, uf.farmer_type, uf.status,
                      uf.last_work, uf.config, ft.work_interval_seconds
               FROM user_farmers uf
               JOIN farmer_types ft ON uf.farmer_type = ft.type_code
        ...
        return [{
            ...
            'last_work': row[4],
            'config': row[5],
            'work_interval_seconds': row[6] or 300
        } for row in rows]
    ...
    async def farmer_work(self, user_id: int) -> Dict:
        ...
        """
        # Получаем фермера
        farmer = await self.fetchone(
            """SELECT farmer_id, farmer_type, config FROM user_farmers 
               WHERE user_id = ? AND status = 'active'""",
            (user_id,)
        )
        
        if not farmer:
            return {"success": False, "message": "Нет активного фермера"}
        
        farmer_id, farmer_type, config_str = farmer
        ...
        for plot in ready_plots:
            ...
            if crop_data:
                ...
                # Очищаем грядку
                await self.execute(
                    """UPDATE plots SET status = 'empty', crop_type = NULL,
                       planted_time = NULL, growth_time_seconds = 0
                       WHERE user_id = ? AND plot_number = ?""",
                    (user_id, plot['number']),
                    commit=True
                )
        
        # Сажаем на пустые грядки
        empty_plots = [p for p in plots if p['status'] == 'empty']
        seeds = await self.get_shop_items('seed')
        
        # Получаем предпочтительную культуру из конфига
        preferred_crop = config.get('preferred_crop')
        
        for plot in empty_plots[:3]:  # Максимум 3 посадки за раз
            # Выбираем культуру
            crop = None
            if preferred_crop:
                crop = next((c for c in seeds if c['item_code'] == preferred_crop), None)
            if not crop:
                # Берём самую дешёвую
                affordable = [c for c in seeds if c.get('buy_price', 0) <= 100]
                if affordable:
                    crop = min(affordable, key=lambda x: x.get('buy_price', 0))
            
            if crop:
                user = await self.get_user(user_id)
                if user and user.get('balance', 0) >= crop.get('buy_price', 0):
                    # Списываем деньги
                    await self.update_balance(user_id, -crop.get('buy_price', 0))
                    # Сажаем
                    await self.plant_crop(user_id, plot['number'], crop['item_code'], crop.get('growth_time', 120))
                    planted += 1
        
        # Обновляем время работы
        await self.execute(
            "UPDATE user_farmers SET last_work = datetime('now') WHERE farmer_id = ?",
            (farmer_id,),
            commit=True
        )
        ...
        # Начисляем заработок
        if earned > 0:
            await self.update_balance(user_id, earned)
        
        # Списываем зарплату (условно 10% от заработка)
        salary = int(earned * 0.1)
        if salary > 0:
            await self.update_balance(user_id, -salary)
        ...
        return {
            ...
            "earned": earned,
            "salary": salary,
            "skipped": planted == 0 and harvested == 0
        }
    
    async def get_user_farmer(self, user_id: int) -> Optional[Dict]:
        """Получает информацию о фермере пользователя"""
        row = await self.fetchone(
            """SELECT uf.farmer_id, uf.farmer_type, uf.status, uf.config,
                      ft.type_name, ft.type_icon
               FROM user_farmers uf
               LEFT JOIN farmer_types ft ON uf.farmer_type = ft.type_code
               WHERE uf.user_id = ?""",
            (user_id,)
        )
