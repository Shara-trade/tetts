...
• Множитель: x{float(user.get('prestige_multiplier') or 1.0):.1f}
...
async def promo_process_handler(message: Message):
    ...
    if result.get("success"):
        ...
        if user:
            text_lines.append("")
            text_lines.append(f"Баланс обновлен: {int(user.get('balance') or 0):,}🪙 | {int(user.get('gems') or 0)}💎")
...
async def claim_all_daily_quests(callback: CallbackQuery):
    ...
    user = await db.get_user(callback.from_user.id)
    if user:
        text_lines.append(f"\nНовый баланс: {int(user.get('balance') or 0):,}🪙 | {int(user.get('gems') or 0)}💎")
...
Стоимость: {cost}💎
Твой баланс: {int(user.get('gems') or 0)}💎
...
━━━━━━━━━━━━━━━━━━━━━━━
📊 <b>СТАТИСТИКА:</b>
🎚️ Уровень: {int(user.get('level') or 1)}
🚜 Престиж: {int(user.get('prestige_level') or 1)} (x{float(user.get('prestige_multiplier') or 1.0):.1f})
💰 Баланс: {int(user.get('balance') or 0):,} 🪙
💎 Кристаллы: {int(user.get('gems') or 0)} 💎
🌾 Собрано: {int(user.get('total_harvested') or 0):,} урожаев
🌱 Посажено: {int(user.get('total_planted') or 0):,} раз
📦 Всего заработано: {int(user.get('total_earned') or 0):,} 🪙

━━━━━━━━━━━━━━━━━━━━━━━
⚙️ <b>НАСТРОЙКИ:</b>
🔔 Уведомления: {'Вкл' if notif_enabled else 'Выкл'}
📊 Лимит переводов: 20% (доступно: {int(int(user.get('balance') or 0) * 0.2):,}🪙/день)"""
...
👤 <b>Основное:</b>
• ID: {user.get('user_id')}
• Уровень: {int(user.get('level') or 1)}
• Опыт: {int(user.get('xp') or 0):,}
...
🌾 <b>Фермерство:</b>
• Собрано урожая: {int(user.get('total_harvested') or 0):,}
• Посажено растений: {int(user.get('total_planted') or 0):,}
• Престиж: {int(user.get('prestige_level') or 1)}
• Множитель: x{float(user.get('prestige_multiplier') or 1.0):.1f}
