...
from admin.keyboards import get_back_keyboard, get_inventory_category_keyboard, get_inventory_keyboard, get_item_detail_keyboard, get_main_keyboard, get_farm_keyboard, get_sell_keyboard, get_shop_items_keyboard, get_shop_keyboard, get_active_quests, get_inventory_item_keyboard, get_achievement_category_keyboard, get_achievement_all_keyboard, get_profile_select_keyboard
...
async def inventory_item_detail(callback: CallbackQuery):
    ...
    # Создаём клавиатуру для предмета инвентаря
...
# ============ ДОПОЛНИТЕЛЬНЫЕ ОБРАБОТЧИКИ ДЛЯ ДОСТИЖЕНИЙ ============

@router.callback_query(F.data == "ach_select_profile")
async def ach_select_profile(callback: CallbackQuery, state: FSMContext):
    ...
    if not completed:
        await callback.answer("❌ У тебя нет выполненных достижений!", show_alert=True)
        return
    
    text = (
        "🏆 <b>ВЫБОР ДОСТИЖЕНИЙ В ПРОФИЛЬ</b>\n\n"
        "Выбери до 3 достижений для отображения в твоём профиле:\n\n"
        "Нажми на достижение, чтобы выбрать/отменить выбор."
    )
    
    # Получаем текущий выбор
    current_selection = await db.get_profile_achievements(callback.from_user.id)
    selected_ids = [a.get('id') for a in current_selection] if current_selection else []
    
    # Сохраняем в state
    await state.update_data(selected_achievements=selected_ids)
    
    # Формируем клавиатуру
    keyboard = get_profile_select_keyboard(completed, selected_ids)
...
async def ach_toggle(callback: CallbackQuery, state: FSMContext):
    ...
    selected = data.get('selected_achievements', [])
    
    # Переключаем
    if ach_id in selected:
        selected.remove(ach_id)
    else:
        if len(selected) >= 3:
            await callback.answer("❌ Максимум 3 достижения!", show_alert=True)
            return
        selected.append(ach_id)
    
    # Обновляем state
    await state.update_data(selected_achievements=selected)
    
    # Обновляем клавиатуру
    db = await get_db()
    completed = await db.get_user_completed_achievements(callback.from_user.id)
    keyboard = get_profile_select_keyboard(completed, selected)
    
    await callback.message.edit_reply_markup(reply_markup=keyboard)
    await callback.answer()


@router.callback_query(F.data == "ach_save_profile")
async def ach_save_profile(callback: CallbackQuery, state: FSMContext):
    """Сохранение выбранных достижений в профиль"""
    db = await get_db()
    
    # Получаем выбор из state
    data = await state.get_data()
    selected = data.get('selected_achievements', [])
    
    # Сохраняем в БД
    await db.set_profile_achievements(callback.from_user.id, selected)
    
    await state.clear()
    await callback.answer("✅ Достижения сохранены!", show_alert=True)
    
    # Возвращаем к достижениям
    await achievements_handler(callback.message)


@router.callback_query(F.data == "ach_history")
async def ach_history(callback: CallbackQuery):
    """История полученных достижений"""
    db = await get_db()
    
    history = await db.get_achievement_history(callback.from_user.id)
    
    text = "📜 <b>ИСТОРИЯ ДОСТИЖЕНИЙ</b>\n\n"
    
    if not history:
        text += "Пока нет полученных достижений.\n\n"
        text += "Продолжай играть, и ты обязательно их получишь! 🏆"
    else:
        for ach in history[:10]:
            icon = ach.get('icon', '🏆')
            name = ach.get('name', '???')
            date = ach.get('completed_at', '')
...
async def ach_view(callback: CallbackQuery):
    """Просмотр детали достижения"""
    try:
    ...
    name = ach.get('name', '???')
    desc = ach.get('description', '')
    reward_coins = ach.get('reward_coins', 0)
    reward_gems = ach.get('reward_gems', 0)
    completed = ach.get('completed', False)
    ...
    if completed:
        if claimed:
            text_lines.append("✅ Награда получена!")
        else:
            text_lines.append("🎁 Награда доступна!")
    else:
        pct = min(100, int(progress / target * 100)) if target > 0 else 0
        text_lines.append(f"📈 Выполнено: {pct}%")
    
    text_lines.append("")
    text_lines.append(f"🎁 Награда: {reward_coins:,}🪙")
    if reward_gems > 0:
        text_lines.append(f"   + {reward_gems}💎")
    ...
    if completed and not claimed:
        keyboard_rows.append([
            InlineKeyboardButton(text="🎁 Забрать награду", callback_data=f"ach_claim_{ach_id}")
        ])
    
    if completed:
        keyboard_rows.append([
            InlineKeyboardButton(text="🏆 В профиль", callback_data=f"ach_to_profile_{ach_id}")
        ])
    
    category_id = ach.get('category_id', '')
    keyboard_rows.append([
        InlineKeyboardButton(text="🔙 Назад", callback_data=f"ach_category_{category_id}")
    ])
...
@router.callback_query(F.data.startswith("ach_claim_"))
async def ach_claim(callback: CallbackQuery):
    """Получение награды за достижение"""
    try:
        ach_id = int(callback.data.replace("ach_claim_", ""))
    except ValueError:
        await callback.answer("❌ Ошибка!")
        return
    ...
    if result.get('success'):
        ...
        text = f"🎉 <b>Награда получена!</b>\n\n💰 +{coins:,}🪙"
        if gems > 0:
            text += f"\n💎 +{gems}"
        
        await callback.answer(text, show_alert=True)
        await ach_view(callback)
    else:
        await callback.answer(result.get('message', "❌ Ошибка!"), show_alert=True)


@router.callback_query(F.data.startswith("ach_to_profile_"))
async def ach_to_profile(callback: CallbackQuery):
    """Добавление достижения в профиль"""
    try:
        ach_id = int(callback.data.replace("ach_to_profile_", ""))
    except ValueError:
        await callback.answer("❌ Ошибка!")
        return
    ...
    # Получаем текущие достижения в профиле
    current = await db.get_profile_achievements(callback.from_user.id)
    current_ids = [a.get('id') for a in current] if current else []
    
    # Добавляем новое, если ещё нет
    if ach_id not in current_ids:
        if len(current_ids) >= 3:
            # Удаляем первое, добавляем новое
            current_ids = current_ids[1:]
        current_ids.append(ach_id)
        await db.set_profile_achievements(callback.from_user.id, current_ids)
    
    await callback.answer("✅ Добавлено в профиль!", show_alert=True)


@router.callback_query(F.data.startswith("ach_page_"))
async def ach_page(callback: CallbackQuery):
    """Пагинация достижений категории"""
    parts = callback.data.split("_")
    if len(parts) < 4:
        await callback.answer("❌ Ошибка!")
        return
    ...
    # Получаем достижения категории (user_id, category_id)
    achievements = await db.get_achievements_by_category(callback.from_user.id, category_id)
    
    # Считаем невостребованные награды
    pending_count = sum(1 for a in achievements if a.get('completed') and not a.get('reward_claimed'))
    
    # Формируем клавиатуру с пагинацией
    keyboard = get_achievement_category_keyboard(category_id, achievements, pending_count=pending_count, page=page)
    
    await callback.message.edit_reply_markup(reply_markup=keyboard)
    await callback.answer()


@router.callback_query(F.data.startswith("ach_all_page_"))
async def ach_all_page(callback: CallbackQuery):
    """Пагинация всех достижений"""
    try:
        page = int(callback.data.replace("ach_all_page_", ""))
    except ValueError:
        page = 0
    ...
    # Формируем клавиатуру с пагинацией
    keyboard = get_achievement_all_keyboard(achievements, page=page)
    
    await callback.message.edit_reply_markup(reply_markup=keyboard)
    await callback.answer()


@router.callback_query(F.data.startswith("ach_sel_page_"))
async def ach_sel_page(callback: CallbackQuery, state: FSMContext):
    """Пагинация выбора достижений для профиля"""
    try:
        page = int(callback.data.replace("ach_sel_page_", ""))
    except ValueError:
        page = 0
    ...
    # Формируем клавиатуру
    keyboard = get_profile_select_keyboard(completed, selected, page=page)
