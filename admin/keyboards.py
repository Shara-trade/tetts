...
def get_active_quests(daily_quests: List[Dict] = None, weekly_quests: List[Dict] = None):
    """Получает клавиатуру активных квестов (объединённая для ежедневных и еженедельных)
    
    Args:
        daily_quests: Список ежедневных квестов
        weekly_quests: Список еженедельных квестов
        
    Returns:
        InlineKeyboardMarkup с квестами
    """
    inline_rows = []
    ...
    if daily_quests:
        inline_rows.append([
            InlineKeyboardButton(text="📜 Ежедневные квесты", callback_data="show_daily_quests")
        ])
        ...
        for quest in daily_quests[:3]:
            ...
            inline_rows.append([
                InlineKeyboardButton(text=f"  {status} {name}", callback_data=f"quest_detail_{quest.get('quest_id', 0)}")
            ])
    
    # Еженедельные квесты
    if weekly_quests:
        inline_rows.append([
            InlineKeyboardButton(text="📅 Еженедельные квесты", callback_data="show_weekly_quests")
        ])
        ...
        for quest in weekly_quests[:2]:
            ...
            status = "✅" if completed else f"{progress}/{target}"
            inline_rows.append([
                InlineKeyboardButton(text=f"  {status} {name}", callback_data=f"quest_detail_{quest.get('quest_id', 0)}_weekly")
            ])
    
    # Кнопки действий
    has_completed = any(q.get('completed') and not q.get('claimed') for q in (daily_quests or []))
    has_completed = has_completed or any(q.get('completed') and not q.get('claimed') for q in (weekly_quests or []))
