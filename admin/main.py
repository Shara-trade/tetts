...
# Импортируем роутеры с обработкой ошибок (относительные импорты из текущей папки admin)
try:
    from .handlers import router as player_router
    print("✅ handlers router загружен")
except Exception as e:
    print(f"❌ Ошибка загрузки handlers: {e}")
    player_router = None

try:
    from .admin_panel_full import router as admin_router
    print("✅ admin_panel_full router загружен")
except Exception as e:
    print(f"❌ Ошибка загрузки admin_panel_full: {e}")
    admin_router = None

try:
    from .achievements_admin import router as achievements_admin_router
    print("✅ achievements_admin router загружен")
except Exception as e:
    print(f"❌ Ошибка загрузки achievements_admin: {e}")
    achievements_admin_router = None
...
async def main():
    ...
    # Подключаем роутеры с проверкой
    if player_router:
        dp.include_router(player_router)
        print("✅ player_router подключен")
    
    if admin_router:
        dp.include_router(admin_router)
        print("✅ admin_router подключен")
    else:
        print("⚠️ admin_router не подключен!")
    
    if achievements_admin_router:
        dp.include_router(achievements_admin_router)
        print("✅ achievements_admin_router подключен")
