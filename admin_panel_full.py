...
async def admin_players_menu(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        "Выбери способ поиска или просмотра:",
...
async def admin_search_by_id(callback: CallbackQuery, state: FSMContext):
    """Поиск по ID"""
    await state.set_state(AdminFSM.search_user_id)
...
@router.message(AdminFSM.search_user_id)
...
async def admin_search_by_username(callback: CallbackQuery, state: FSMContext):
    """Поиск по username"""
    await state.set_state(AdminFSM.search_username)
...
@router.message(AdminFSM.search_username)
...
async def admin_confirm_role(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        await db.execute(
            """INSERT INTO admin_users (user_id, role, added_by, added_at)
               VALUES (?, ?, ?, datetime('now'))""",
...
@router.callback_query(F.data.in_([
    ...
    "admin_plants_trash", "admin_plants_stats", "admin_promo_stats",
    "admin_promo_active", "admin_promo_expired", "admin_list_promo_0",
    "logs_analytics", "admin_remove_role", "admin_broadcast_audience"
]))
